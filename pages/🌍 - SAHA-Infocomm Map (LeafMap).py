...
m = leafmap.Map(center=[-29, 26], zoom=6)
layer_res = []
selections = []
if st.sidebar.button("Create Map and Data"):
    ...
    for layer in range(layer_nums):
        ...
        if len(res) > 0:
            ...
            cols2 = st.columns(2)
            if len(data) > 0:
                final_data = json.loads(data[["place", "full_name", "description", "coordinates"]].to_json())
                ...
                for idx, row in final_df.iterrows():
                    final_df.at[idx, "place"] = row.place[0]
                    # final_df.at[idx, "frequency"] = frequencies[row.place[0]]
                    final_df.at[idx, "longitude"] = row.coordinates[0]
                    final_df.at[idx, "latitude"] = row.coordinates[1]
